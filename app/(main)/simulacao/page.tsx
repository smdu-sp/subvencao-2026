...
const INTEGRACAO_OPCOES = [
  ...
  { id: "mar",                label: "Disponibilização de área para o Museu de Arte de Rua (MAR)",                                             pontos:  5 },
  { id: "prox_credenciados",  label: "Proximidade de projetos credenciados ou outorgados para Subvenção Econômica",                            pontos:  5 },
  { id: "prox_obras",         label: "Proximidade de obras de construção, reforma ou requalificação sem apoio da Subvenção Econômica",         pontos:  5 },
];
...
const TECNOLOGIA_OPCOES = [
  ...
  { id: "certificacao",    label: "Incentivo à certificação de edificação sustentável", pontos:  5 },
];
  ...
  return (
    <div className="flex items-stretch mb-5">
      ...
      <div className="flex items-center gap-3 bg-gray-100 px-4 py-3 rounded-r-sm flex-1">
        <span className={`${dmSans.className} text-xs font-bold uppercase tracking-widest text-[#0a3297]`}>
          {numero}
        </span>
        <span className={`${dmSans.className} text-[19px] font-bold text-black`}>{titulo}</span>
      </div>
    </div>
  );
}
...
function TituloSubsecao({ children }: { children: React.ReactNode }) {
  return (
    <h3 className={`${dmSans.className} text-[17px] font-bold text-[#0a3297] mt-7 mb-2`}>
      {children}
    </h3>
  );
}
...
function TagPontuacao({ valor, maximo }: { valor: number; maximo?: number }) {
  return (
    <span className="inline-flex items-center gap-1 bg-gray-100 rounded-sm px-2.5 py-1 mt-3 text-[13px]">
      ...
      <span className={`font-bold text-[#0a3297]`}>{NUM.format(valor)} pts</span>
      {maximo !== undefined && (
...
export default function Simulacao() {
  ...
  const [uso, setUso] = useState<Record<UsoKey, string>>({
    his1: "", his2: "", hmp: "", r2v: "", nao_residencial: "",
  });
  ...
  // ─── Cálculos ──────────────────────────────────────────────────────────────
  const calc = useMemo(() => {
    const somaUso = USO_OPCOES.reduce((s, o) => s + (parseFloat(uso[o.id]) || 0), 0);
    ...
    const p2 =
      patrimonio === "tombado"   ? 20 :
      patrimonio === "envoltoria" ?  5 : 0;

    // P3 – Integração
    const p3 = INTEGRACAO_OPCOES.reduce(
      (s, o) => (integracaoMarcados.includes(o.id) ? s + o.pontos : s), 0
    );

    // P4 – Tecnologia
    const p4 = TECNOLOGIA_OPCOES.reduce(
      (s, o) => (tecnologiaMarcados.includes(o.id) ? s + o.pontos : s), 0
    );
    ...
    return { somaUso, p1, p1Raw, p2, p3, p4, P, sBase, numBonus, S, vpNum, vnNum, Vs };
  }, [uso, patrimonio, integracaoMarcados, tecnologiaMarcados, bonusA, bonusB, bonusC, vp, vn]);
      ...
      >
        ...
        <header className="mb-10">
          ...
          <p className="text-[18px] mt-2 text-gray-700 leading-snug">
            Preencha os campos abaixo para estimar a pontuação e o valor de subvenção do seu
            projeto, conforme o{" "}
            <strong>Decreto Municipal nº 62.878/2023</strong>.
          </p>
          <p className="text-[13px] mt-2 text-gray-400">
            * Esta simulação é apenas informativa. Os valores finais dependem de análise técnica da SMUL.
          </p>
        </header>
        ...
        <section aria-labelledby="quadro1-titulo" className="mb-10">
          ...
          <CabecalhoSecao numero="Quadro 1" titulo="Critérios de Relevante Interesse Urbanístico" />
          ...
          <fieldset>
            ...
            <p className="text-[15px] text-gray-600 mb-4">
              Informe a porcentagem de área destinada a cada uso. A soma deve ser igual a{" "}
              <strong>100%</strong>. A pontuação é calculada proporcionalmente (máximo de{" "}
                ...
                <div key={opt.id} className="flex items-center gap-4 flex-wrap">
                  ...
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      value={uso[opt.id]}
                      onChange={(e) =>
                        setUso((prev) => ({ ...prev, [opt.id]: e.target.value }))
                      }
          ...
          <fieldset className="mt-2">
            <legend>
              <TituloSubsecao>Valorização do Patrimônio Histórico</TituloSubsecao>
            </legend>
            <p className="text-[15px] text-gray-600 mb-4">
              Selecione a condição patrimonial do imóvel (máximo de <strong>20 pontos</strong>).
              As opções não são cumulativas entre si.
            </p>
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                },
                {
                  ...
                  label: "Imóvel não tombado e não localizado em área envoltória",
                  pontos: 0,
                },
              ].map((opt) => (
                <label
        ...
        <section aria-labelledby="quadro2-titulo" className="mb-10">
          ...
          <CabecalhoSecao numero="Quadro 2" titulo="Elementos Geradores de Externalidades Positivas" />

          {/* Integração com a Dinâmica Urbana */}
          <fieldset>
            <legend>
              <TituloSubsecao>Integração do Projeto com a Dinâmica Urbana</TituloSubsecao>
            </legend>
            <p className="text-[15px] text-gray-600 mb-4">
              ...
              {INTEGRACAO_OPCOES.map((opt) => (
                <label key={opt.id} className="flex items-start gap-3 cursor-pointer">
                  <input
                    ...
                    onChange={() =>
                      alternarLista(opt.id, integracaoMarcados, setIntegracaoMarcados)
                    }
                    className="mt-0.5 h-5 w-5 accent-[#0a3297] shrink-0"
          ...
          <fieldset className="mt-2">
            <legend>
              <TituloSubsecao>Tecnologia e Procedimentos Construtivos Sustentáveis</TituloSubsecao>
            </legend>
            <p className="text-[15px] text-gray-600 mb-4">
              Selecione todos os elementos aplicáveis. Os itens são{" "}
              <strong>cumulativos</strong> entre si.
              ...
              {TECNOLOGIA_OPCOES.map((opt) => (
                <label key={opt.id} className="flex items-start gap-3 cursor-pointer">
                  <input
                    ...
                    onChange={() =>
                      alternarLista(opt.id, tecnologiaMarcados, setTecnologiaMarcados)
                    }
                    className="mt-0.5 h-5 w-5 accent-[#0a3297] shrink-0"
                    aria-label={opt.label}
                  />
        ...
        <section aria-labelledby="bonificacao-titulo" className="mb-10">
          ...
          <CabecalhoSecao numero="Bonificação" titulo="Acréscimo no Percentual de Subvenção" />

          <p className="text-[15px] text-gray-600 mb-5">
            Cada item válido acresce <strong>+5%</strong> ao percentual de subvenção (S),
            observando o limite máximo de 25%. Os itens a, b e c são{" "}
            <strong>cumulativos entre si</strong>.
          </p>
          ...
          <div className="mb-4">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              />
              <span className="text-[16px] leading-snug">
                <strong>(a)</strong> Empreendimento com área total de edificação de até{" "}
          ...
          <div className="mb-5">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              />
              <span className="text-[16px] leading-snug">
                <strong>(b)</strong> Mudança de uso de imóvel enquadrado nas categorias{" "}
          ...
          <fieldset>
            ...
            <div className="ml-5 flex flex-col gap-3">
              {[
                ...
                { valor: "his1_30" as BonusC, label: "No mínimo 30% da área total composta por HIS-1" },
                { valor: "his2_40" as BonusC, label: "No mínimo 40% da área total composta por HIS-2" },
                { valor: "hmp_50"  as BonusC, label: "No mínimo 50% da área total composta por HMP"  },
              ].map((opt) => (
                <label key={opt.valor} className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="radio"
                    name="bonusC"
                    value={opt.valor}
          ...
          <div className="mt-4 inline-flex items-center gap-2 bg-gray-100 rounded-sm px-3 py-1.5 text-[13px]">
            ...
            <span className="font-bold text-[#0a3297]">+{calc.numBonus * 5}%</span>
          </div>
        </section>
        ...
        <section aria-labelledby="valores-titulo" className="mb-10">
          ...
          <CabecalhoSecao numero="Valores" titulo="Valores do Projeto (opcional)" />

          <p className="text-[15px] text-gray-600 mb-6">
            Preencha para calcular o <strong>montante estimado</strong> da subvenção em reais.
            Os valores devem seguir os parâmetros da ABNT NBR 12.721.
          </p>
          ...
          <div className="flex flex-col gap-6 max-w-sm">
            ...
            {vpMenorQueVn && (
              <p className="text-[13px] text-red-600 font-semibold" role="alert">
                ⚠ O valor não computável (Vn) é maior que o valor total do projeto (Vp).
              </p>
            )}
          </div>
        </section>
          ...
          <div className="bg-[#e7e6f0] rounded-sm p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-white rounded-sm p-4 flex flex-col items-center text-center shadow-sm">
                <p className={`${dmSans.className} text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-1`}>
                  Pontuação Total
                </p>
                <p className={`${dmSans.className} text-[38px] font-bold text-[#0a3297] leading-none`}>
                  {NUM.format(calc.P)}
                </p>
              ...
              >
                ...
                {temFinanceiro && !vpMenorQueVn ? (
                  <>
                    <p className={`${dmSans.className} text-[26px] font-bold text-green-700 leading-tight`}>
                      {BRL.format(calc.Vs)}
                    </p>
                    <p className="text-[11px] text-gray-400 mt-1">
                      {NUM.format(calc.S)}% × {BRL.format(calc.vpNum - calc.vnNum)}
                    </p>
                  </>
                ) : (
                  <>
                    <p className={`${dmSans.className} text-[38px] font-bold text-gray-200 leading-none`}>—</p>
                    <p className="text-[12px] text-gray-400 mt-1">
                      {vpMenorQueVn ? "Valores inválidos" : "Preencha Vp e Vn acima"}
                    </p>
                  </>
                )}
              </div>
            ...
            <div className="bg-white rounded-sm overflow-hidden shadow-sm">
              ...
              <div
                ...
                className={`grid transition-all duration-300 ease-out ${
                  detalheAberto ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
                }`}
              >
                <div className="overflow-hidden">
                  <div className="px-4 pb-4 pt-1 border-t border-gray-100">
                    <table className="w-full text-[14px]" aria-label="Detalhamento da pontuação">
                      <thead>
                        <tr className="text-left text-[11px] uppercase tracking-wide text-gray-400 border-b border-gray-100">
                          <th className="py-1.5 font-semibold">Critério</th>
                          <th className="py-1.5 font-semibold text-right">Pontos obtidos</th>
                          <th className="py-1.5 font-semibold text-right">Máximo</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50">
                        <tr>
                          <td className="py-1.5">Uso do Imóvel</td>
                          <td className="py-1.5 text-right font-semibold">{NUM.format(calc.p1)}</td>
                          <td className="py-1.5 text-right text-gray-400">30</td>
                        </tr>
                        <tr>
                          <td className="py-1.5">Valorização do Patrimônio Histórico</td>
                          <td className="py-1.5 text-right font-semibold">{NUM.format(calc.p2)}</td>
                          <td className="py-1.5 text-right text-gray-400">20</td>
                        </tr>
                        <tr>
                          <td className="py-1.5">Integração com a Dinâmica Urbana</td>
                          <td className="py-1.5 text-right font-semibold">{NUM.format(calc.p3)}</td>
                          <td className="py-1.5 text-right text-gray-400">55</td>
                        </tr>
                        <tr>
                          <td className="py-1.5">Tecnologia Construtiva Sustentável</td>
                          <td className="py-1.5 text-right font-semibold">{NUM.format(calc.p4)}</td>
                          <td className="py-1.5 text-right text-gray-400">25</td>
                        </tr>
                        <tr className="border-t border-gray-200 font-bold">
                          <td className="py-1.5">Pontuação total (P)</td>
                          <td className="py-1.5 text-right text-[#0a3297]">{NUM.format(calc.P)}</td>
                    ...
                    <table className="w-full text-[14px]" aria-label="Cálculo do percentual de subvenção">
                      <tbody className="divide-y divide-gray-50">
                        <tr>
                          <td className="py-1.5">S base = P × 0,25</td>
                          <td className="py-1.5 text-right font-semibold">{NUM.format(calc.sBase)}%</td>
                        </tr>
                        <tr>
                          <td className="py-1.5">
                            Bonificação ({calc.numBonus} {calc.numBonus === 1 ? "item" : "itens"} × 5%)
                          </td>
                          <td className="py-1.5 text-right font-semibold">+{calc.numBonus * 5}%</td>
                        </tr>
                        <tr className="font-bold">
                          <td className="py-1.5">Percentual de subvenção (S)</td>
                          <td className="py-1.5 text-right text-[#0a3297]">{NUM.format(calc.S)}%</td>
                        </tr>
                        {temFinanceiro && !vpMenorQueVn && (
                          <>
                            <tr>
                              <td className="py-1.5">Vp − Vn</td>
                              <td className="py-1.5 text-right font-semibold">{BRL.format(calc.vpNum - calc.vnNum)}</td>
                            </tr>
                            <tr className="font-bold">
                              <td className="py-1.5">Vs = (Vp − Vn) × S/100</td>
                              <td className="py-1.5 text-right text-green-700">{BRL.format(calc.Vs)}</td>
                            </tr>
                          </>
                        )}
                      </tbody>
            ...
            <p className="mt-4 text-[12px] text-gray-500 leading-relaxed">
              ...
              Fórmulas: <em>S = P × 0,25</em> e <em>Vs = (Vp − Vn) × (S/100)</em>.
              Os resultados são estimativas e não substituem a análise técnica da SMUL.
            </p>
          </div>
        </section>
